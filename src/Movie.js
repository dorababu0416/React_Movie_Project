...
export function Movie({ poster, name, rating, trailer, summary, index }) {
  const [show, setshow]=useState(true);
  const history1=useHistory();
  const [isOpen, setOpen] = useState(false);
  return (<Card className="movie-container">
    <CardMedia title="click for movie info">
    <img className="movie-poster" onClick={()=>history1.push(`/displaymovielist/${index}`)} src={poster} alt={name} />
    </CardMedia>
    <CardContent>
    <div className="movie-specs">
      <h3 className="movie-name">{name} <IconButton aria-label="delete" color="primary" onClick={()=>setshow(!show)}>
